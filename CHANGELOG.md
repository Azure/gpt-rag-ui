...
### Added
- **Secure opt-in Chainlit Copilot embedding ([Azure/GPT-RAG#556](https://github.com/Azure/GPT-RAG/issues/556))**: Added `POST /copilot/auth/bootstrap`, `POST /copilot/auth/logout`, and `GET /api/download/{grant_token}`; bounded LRU server-side Entra/Chainlit token state behind an opaque `HttpOnly` session cookie capped by Entra expiry; exact portal-origin enforcement that never trusts `Referer`; process-local bootstrap throttling with `429` and `Retry-After`; authenticated principal/conversation/blob-bound SHA-256 citation grants with canonical UUID conversation IDs, one-hour expiry, chunked Blob streaming, and default-denied shared containers; `tid:oid` thread ownership and account-switch isolation; minimum-strength persistent Chainlit signing-secret enforcement; default-denied browser bridges; and an operator guide covering Chainlit 2.9.4 Shadow DOM/CSP, single-revision affinity, third-party-cookie, cross-tab, and trusted-ingress limitations. Standalone OAuth remains a separate fail-closed policy.

### Fixed
- **Bound Copilot Socket.IO transports:** Enforced a fixed limit of four physical Socket.IO connections per opaque Copilot session, explicitly disconnected superseded transports before Chainlit session restoration, and made logout, expiry, eviction, account switch, and same-principal session replacement invalidate all tracked transports and cancel associated active Chainlit tasks.
