...
### Added
- **Secure opt-in Chainlit Copilot embedding ([Azure/GPT-RAG#556](https://github.com/Azure/GPT-RAG/issues/556))**: Added `POST /copilot/auth/bootstrap`, `POST /copilot/auth/logout`, and `GET /api/download/{grant_token}`; bounded server-side Entra/Chainlit token state behind an opaque `HttpOnly` session cookie capped by Entra expiry; exact portal-origin enforcement that never trusts `Referer`; process-local bootstrap throttling with `429` and `Retry-After`; authenticated principal/conversation/blob-bound SHA-256 citation grants with default-denied shared containers; `tid:oid` thread ownership and account-switch isolation; persistent Chainlit signing-secret enforcement; default-denied browser bridges; and an operator guide covering Chainlit 2.9.4 Shadow DOM/CSP, affinity, third-party-cookie, and trusted-ingress limitations. Standalone OAuth remains a separate fail-closed policy.
