...
### Added
- **Secure opt-in Chainlit Copilot embedding ([Azure/GPT-RAG#556](https://github.com/Azure/GPT-RAG/issues/556))**: Added explicit `anonymous` and `entra` modes with no default or downgrade, same-origin reverse-proxy support through a validated `CHAINLIT_ROOT_PATH`, exact-origin enforcement, v2 Entra `azp` portal-client allow-listing, bounded opaque `HttpOnly` sessions, bootstrap/logout endpoints, default-denied browser bridges, and principal/conversation/blob-bound citation downloads. Anonymous sessions use ephemeral identities and intentionally exclude durable threads, user-bound uploads, and authenticated downloads. Standalone authentication remains independent, and the operator guide documents the exact path, proxy, lifecycle, Chainlit 2.9.4, and one-replica contracts.
