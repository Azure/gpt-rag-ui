...
### Fixed
- **ACR remote build log streaming on Windows**: `scripts/deploy.ps1` now forces UTF-8 console/Python output before invoking Azure CLI so `az acr build` log streaming does not fail on Unicode build output (for example Vite's checkmark) when running from the jumpbox.
