...
### Added
- **Opt-in hosted-agent cross-version continuity (`HOSTED_CONTINUITY_ENABLED`, default `false`)**: Added an alternative, BFF-owned continuity model for the `CHAT_BACKEND=hosted_agent` runtime so multi-turn conversations survive hosted-runtime version upgrades. When enabled, this UI — not the hosted runtime — becomes the exclusive owner of the Foundry managed Conversation used for continuity: it creates, reads, appends to, and deletes the managed conversation through the standard Conversations REST surface under the caller's own delegated (OBO) identity, while the hosted runtime itself stays fully stateless and history-blind (it never receives a `conversation_id` or `previous_response_id` and needs zero Conversations RBAC). Ownership is bound with a BFF-minted, signed opaque capability tying the caller's validated Entra `oid`, the managed conversation id, and a key id together (`hosted_conversation_capability.py`, built on the existing `itsdangerous` signing pattern already used by `download_security.py`); a bad signature, expired token, retired signing key, or `oid` mismatch are all rejected identically — the failure never reveals which check failed, so validation cannot be used as an existence oracle, and a capability is only ever minted around a conversation this UI itself just created for the current authenticated user, never around a caller-supplied id. Every turn acquires a one-in-flight lock per conversation (`hosted_conversation_store.py`), reads ordered history from the system-of-record, applies an explicit bounded-history policy (drop-oldest by item count then by an estimated token budget, never dropping the single most recent item), and appends the completed turn back fail-closed: an append or read failure raises `ContinuityPersistenceError` and the turn is never presented as a successfully saved completion, while an idempotent client turn id prevents a duplicate append on retry. While the feature flag is `false` (the default for all existing deployments), none of this new code path runs and the current hosted-agent behavior — a Foundry-issued `conversation_id` round-tripped by the hosted runtime and full history resent by the Chainlit client each turn — is completely unchanged. New configuration: `HOSTED_CONVERSATION_OWNER_BINDING` (`capability` default; `delegated` is a reserved, unimplemented alternative name that stays inert/rejected unless `HOSTED_CONVERSATION_OWNER_BINDING_VALIDATED=true` is also explicitly set), `HOSTED_CONVERSATION_CAPABILITY_KEY`/`_KEY_ID`/`_TTL_SECONDS`, `HOSTED_HISTORY_MAX_ITEMS`/`_MAX_TOKENS`/`_TRUNCATION` (only `drop_oldest` supported), and `HOSTED_CONVERSATION_STORE_BASE_URL`/`_RESOURCE_SCOPE` (the latter falls back to the existing `HOSTED_AGENT_RESOURCE_SCOPE`). See the README's "Optional: hosted-agent cross-version continuity" section for the full settings table and residual operational notes (the one-in-flight lock and idempotency cache are process-local, not shared across replicas; the exact deployed Foundry Conversations REST path/API version should be validated against the target resource before enabling this feature in any environment).
