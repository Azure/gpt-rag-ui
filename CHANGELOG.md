...
### Added
- **Chainlit hosted-agent BFF path ([Azure/GPT-RAG#587](https://github.com/Azure/GPT-RAG/issues/587))**: Added a server-side `hosted_agent` chat backend selected by the `CHAT_BACKEND` configuration key (default: `orchestrator`). When `CHAT_BACKEND=hosted_agent`, the UI posts `InvocationRequest` payloads to `POST /invocations` on the hosted orchestrator and streams the Responses SSE contract: `response.created` (managed `conversation_id`), `response.output_text.delta` (`delta` field), `response.output_text.annotation.added` (citations), `response.function_call_arguments.delta/.done` (tool activity), `response.completed` (termination), `response.cancelled`, and `error`. Conversation continuity is maintained via the server-assigned `conversation_id` only; no fabricated thread-ID is persisted. All credentials are obtained server-side via managed identity using a module-level reusable `ChainedTokenCredential`; authentication is fail-fast — requests are never sent if `HOSTED_AGENT_RESOURCE_SCOPE` is absent or if token acquisition fails (the ARM audience is not an accepted default). A finite SSE idle timeout (120 s) is enforced. No user tokens or caller-identity assertions are forwarded to the backend; end-user document authorisation is governed by the Toolbox/native identity path. Configuration validation at startup raises a clear error when `HOSTED_AGENT_BASE_URL` or `HOSTED_AGENT_RESOURCE_SCOPE` is missing, with no silent fallback to the classic orchestrator path. Live identity/authorisation validation is tracked as a separate follow-on gate ([Azure/GPT-RAG#591](https://github.com/Azure/GPT-RAG/issues/591)).
