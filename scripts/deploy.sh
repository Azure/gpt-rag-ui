...
imageRef="${containerRegistryLoginServer}/azure-gpt-rag/frontend:${tag}"
...
if [[ "$buildMode" == "local" ]]; then
  ...
  acr_build_args=(acr build --registry "${containerRegistryName}" --image "azure-gpt-rag/frontend:${tag}" --file Dockerfile)
